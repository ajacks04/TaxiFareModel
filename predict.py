...
GCP_BUCKET_TEST_PATH = 'gs://le-wagon-bootcamp-305807/test.csv'
GCP_MODEL_PATH = 'gs://le-wagon-bootcamp-305807/models/taxi-fare-model/model.joblib.sav'



def get_test_data(nrows, data="full"):
    ...
    if data == "local":
        ...
        df = pd.read_csv(GCP_BUCKET_TEST_PATH)
    else:
        df = pd.read_csv(GCP_BUCKET_TEST_PATH, nrows=nrows)
    return df


def get_model(path_to_joblib):
    pipeline = joblib.load(GCP_MODEL_PATH)
    return pipeline
